...
public class Pista {
	...
	public Pista(int numero) {
		...
		//Llamar al m�todo para posicionar los obst�culos
	...
	//M�todo para posicionar obst�culos
	private void posicionarObstaculos() {
		...
		//Los obst�culos se van posicionando a partir del lugar donde acaba el obst�culo anterior (con un peque�o margen para que no aparezcan pegados)
		int j=160;
	...
	//M�todo para determinar qu� ha ocurrido en el turno del piloto de esta pista
	public void determinacionObstaculos(Vehiculo participante, VentanaCarrera ventana) {
		
		//Bucle que recorre el array de obst�culos de la pista en cuesti�n y aplica los cambios a la posici�n del participante
		for (int j = 0; j < obstaculos.length; j++) {
			
			//Si una de las ruedas o gran parte del veh�culo del participante cae en el obst�culo se aplicar� su efecto y se imprimir� un mensaje
			if (participante.getPosicion() +5 >= this.obstaculos[j].getLugarInicio() && participante.getPosicion() +5 <= this.obstaculos[j].getLugarFin()|| 
				...
				//Si el veh�culo ha ca�do en una rampa, la rampa lo mover� hacia delante
				...
				//Si el veh�culo ha ca�do en una mancha se le restar� el valor de la mancha
				else {
					
					JOptionPane.showMessageDialog(null, "El veh�culo de " + participante.getPiloto() + " resbala con la mancha de aceite, por lo que retrocede "
							+ this.obstaculos[j].getValor() + " metros!" );
